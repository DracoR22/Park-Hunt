...
import { Address } from 'src/app/addresses/graphql/entity/address.entity'
import { Company } from 'src/app/companies/graphql/entity/company.entity'
import { Verification } from 'src/app/verifications/graphql/entity/verification.entity'
...
import { SlotWhereInput } from 'src/app/slots/graphql/dtos/where.args'
...
export class GaragesResolver {
  ...
  async createGarage(@Args('createGarageInput') args: CreateGarageInput, @GetUser() user: GetUserType) {
    ...
    if (!company?.id) {
      throw new BadRequestException('No company associated with the manager id.')
    }
  ...
  async searchGarages(
    ...
    if (startDate.getTime() > endDate.getTime()) {
      throw new BadRequestException('Start time should be earlier than the end time.')
    }
