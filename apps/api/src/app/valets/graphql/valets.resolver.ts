...
import { Booking } from 'src/app/bookings/graphql/entity/booking.entity'
...
export class ValetsResolver {
  ...
  async companyValets(@Args() args: FindManyValetArgs, @GetUser() user: GetUserType) {
    const company = await this.prisma.company.findFirst({
      where: { Managers: { some: { uid: user.uid } } },
    })
  ...
  @AllowAuthenticated()
  @Query(() => Number)
  async companyValetsTotal(@Args('where', { nullable: true }) where: ValetWhereInput, @GetUser() user: GetUserType) {
    const company = await this.prisma.company.findFirst({
      where: { Managers: { some: { uid: user.uid } } },
    })
  ...
  async valetPickups(@Args() { skip, take }: PaginationInput, @GetUser() user: GetUserType) {
  ...
  async valetDrops(@Args() { skip, take }: PaginationInput, @GetUser() user: GetUserType) {
    const valet = await this.valetsService.validValet(user.uid)
